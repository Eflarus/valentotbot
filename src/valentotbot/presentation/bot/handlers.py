...
async def handle_start_with_link(update: Update, context: ContextTypes.DEFAULT_TYPE, slug: str) -> None:
    ...
    if invalid_link or link is None or user_id is None:
        await update.message.reply_text("Ссылка недействительна или выключена.")  # type: ignore[union-attr]
        return

    prompt_parts = ["Отправь анонимное сообщение."]
    if link.prompt:
        prompt_parts.append(f"Промпт: {link.prompt}")
    prompt_parts.append("Выбери режим раскрытия автора:")
    ...
    async with session_maker() as session:
        ...
        if stats.link_stats:
            for item in stats.link_stats:
                lines.append(
                    f"- {item.label}: сообщений {item.messages_count}, уникальных отправителей {item.unique_senders}"
                )
        else:
...
async def text_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("Для отправки анонимного сообщения используйте ссылку.")
...
async def handle_message_text(
    ...
    if len(text) < 1 or len(text) > 2000:
        await update.message.reply_text("Сообщение должно быть от 1 до 2000 символов.")  # type: ignore[union-attr]
        return
    ...
    await update.message.reply_text("Ваше сообщение отправлено!")  # type: ignore[union-attr]
    ...
    if recipient_chat_id is not None and open_token_value is not None:
        ...
        keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton("Открыть валентинку", callback_data=open_token_value)]]
        )
        try:
...
async def non_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if update.message is None:
        return
    await update.message.reply_text("Доступны только текстовые сообщения.")
...
async def render_messages_page(
    ...
    async with session_maker() as session:
        ...
        if user is None:
            ...
            if target:
                await target.reply_text("Пользователь не найден.")  # type: ignore[union-attr]
            return
        ...
        if link_slug:
            ...
            if link is None or link.owner_user_id != user.id:
                text = "Ссылка не найдена или недоступна."
        ...
        if not messages:
            text = "Сообщений не найдено по выбранным фильтрам."
            if via_callback and update.callback_query is not None:
                await update.callback_query.edit_message_text(text)
            elif update.message:
                await update.message.reply_text(text)  # type: ignore[union-attr]
